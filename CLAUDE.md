...
**@FocusState bypasses animation transactions**: `@FocusState` changes force an immediate layout pass — `.animation(value:)` on derived values only animates rendering, not layout, and `withAnimation` on the focus assignment is ignored. Fix: mirror the focus state into a regular `@State` updated inside `withAnimation` in an `onChange(of: focus)` handler. Views read from the `@State` for visual/layout properties and use `@FocusState` only for `.focused()` bindings.
